import mongoose from "mongoose";
import { readFileSync } from "fs";
import { resolve } from "path";

const lines = readFileSync(resolve(process.cwd(), ".env"), "utf8").split("\n");
for (const line of lines) {
    const m = line.match(/^\s*([^#=\s][^=]*?)\s*=\s*(.*)\s*$/);
    if (m) { const k = m[1], v = m[2].replace(/^["']|["']$/g, ""); if (!process.env[k]) process.env[k] = v; }
}
await mongoose.connect(process.env.MONGODB_URI);

const Game = mongoose.model("Game", new mongoose.Schema(
    { teamA: { name: String, score: Number }, teamB: { name: String, score: Number }, status: String, date: Date },
    { strict: false }
));

const start = new Date("2026-05-14T00:00:00Z");
const end   = new Date("2026-05-20T23:59:59Z");
const games = await Game.find({ date: { $gte: start, $lte: end } }).lean();
console.log("Found:", games.length);
for (const g of games) {
    console.log(String(g._id).padEnd(28), g.date.toISOString().slice(0,10), g.status?.padEnd(12), (g.teamA?.name||"?").padEnd(30), String(g.teamA?.score ?? "null").padEnd(6), "vs", (g.teamB?.name||"?").padEnd(30), g.teamB?.score ?? "null");
}
await mongoose.disconnect();
